...
des = {
    ...
    'hướng dẫn sử dụng': [
        "📩 Gửi lệnh pollwar <hành động> để quản lý cuộc khảo sát.",
        "📌 Ví dụ: pollwar on @nguoitag để bắt đầu cuộc khảo sát tự động cho người được tag, warpoll off để dừng cuộc khảo sát.",
...
def handle_warpoll_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    if len(command_parts) < 2:
        usage = (
            "Sử dụng lệnh:\n"
            "• 'pollwar on <tag người dùng>' để bắt đầu cuộc khảo sát,\n"
            "• 'pollwar off' để dừng cuộc khảo sát."
        )
    ...
    if action != "on":
        usage = (
            "Lệnh không hợp lệ.\n"
            "Sử dụng: 'pollwar on <tag người dùng>' để bắt đầu cuộc khảo sát hoặc 'pollwar off' để dừng."
        )
        client.replyMessage(Message(text=usage), message_object, thread_id, thread_type)
        return
