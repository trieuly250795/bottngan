...
des = {
    ...
    'hướng dẫn sử dụng': [
        "📩 Gửi lệnh senduser <user_id> <message> <times> để gửi tin nhắn riêng đến người dùng với số lần lặp.",
        "📌 Ví dụ: senduser 123456789 Hello 5 để gửi tin nhắn 'Hello' đến người dùng có ID 123456789 5 lần.",
...
def handle_senduser_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    try:
        ...
        if len(command_parts) < 3:
            client.replyMessage(Message(text="Lệnh không hợp lệ. Vui lòng sử dụng định dạng: senduser <user_id> <message> <times>"), message_object, thread_id, thread_type)
            return
