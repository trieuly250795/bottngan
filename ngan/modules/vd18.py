...
des = {
    'version': "1.0.2",
    'credits': "ROSY ",
    'description': "Gửi video gái từ tệp vd18.txt"
}
...
def handle_vd18_command(message, message_object, thread_id, thread_type, author_id, client):
    ...
    uptime_message = "Video 18+ của bạn đây, chúc ngon miệng"
    message_to_send = Message(text=uptime_message)
    
    # Đọc các link video từ tệp txt
    video_links = read_video_links('vd18.txt')  # Đường dẫn tệp txt của bạn
...
def get_mitaizl():
    return {
        'vd18': handle_vd18_command  # Đã thay đổi tên từ 'vd18v3' thành 'vd18'
    }
